...
const registerUser = () => {
  const body = {
    ...
    email: props.userDetail.email_address,
    otp_token: otp.value,
  }
  for (const item in body) {
    if (!body[item]) delete body[item]
  }
  api.post(REGISTER.personalAccount, body)
    .then(res => {
      emit("update:user-detail", { ...props.userDetail, otp_token: otp.value });
      emit("update:component", 3);
      mainStore.setToken(res.data.jwt.access_token)
